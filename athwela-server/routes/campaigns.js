...
router.get('/sort/:sort/count', (req, res) => {
    ...
    if (sortTo == "date") {
        Campaign.find({ published: 'true' }).sort({ "deadline": -1 }).count((err, count) => {
            if (!err) {
    ...
    // sort by name data count
        ...
        Campaign.find({ published: 'true' }).sort({ [sortTo]: -1 }).count((err, count) => {
            if (!err) {
                res.send({ success: true, sortCount: count });
            } else {
                res.send({ success: false, error: err });
    ...
    // sort by trending data count
        ...
        Campaign.find({ published: 'true' }).sort({ "deadline": -1, "comments": -1 }).count((err, count) => {
            if (!err) {
                res.send({ success: true, sortCount: count });
            } else {
                res.send({ success: false, error: err });
